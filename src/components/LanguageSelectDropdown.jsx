    ...
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <div className="flex items-center space-x-1.5">
        ...
        {showLocationPicker && (
          <div className="relative">
            <button
              ...
              className="flex items-center space-x-1 px-2.5 py-1.5 bg-emerald-50/80 dark:bg-emerald-950/60 border border-emerald-200/90 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 rounded-xl text-xs font-semibold hover:bg-emerald-100/70 transition-colors shadow-2xs"
            ...
            {showLocMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 p-2 max-h-64 overflow-y-auto custom-scrollbar animate-fade-in">
                <div className="px-2 py-1.5 text-[11px] font-bold font-heading text-slate-400 uppercase tracking-wider flex items-center justify-between border-b border-slate-100 dark:border-slate-800 mb-1">
        ...
        <button
          ...
          className={`flex items-center space-x-1.5 px-3 py-1.5 bg-white/90 dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-800 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-2xs ${
            compact ? 'px-2 py-1' : ''
          }`}
        ...
        >
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400 hidden sm:inline">({activeLang.name})</span>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-72 sm:w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl z-50 p-3 max-h-[80vh] overflow-y-auto custom-scrollbar animate-fade-in">
          
          {/* Header & Search */}
          <div className="mb-2.5 pb-2 border-b border-slate-100 dark:border-slate-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold font-heading text-slate-900 dark:text-white flex items-center gap-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
              />
            </div>
          </div>
          ...
          {filteredPriority.length > 0 && (
            <div className="mb-3">
              <div className="flex items-center space-x-1 px-1 mb-1.5 text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">
                    ...
                    <button
                      ...
                      className={`w-full text-left p-2 rounded-xl transition-colors flex items-center justify-between ${
                        isSelected
                          ...
                          : 'bg-emerald-50/50 dark:bg-emerald-950/30 text-slate-900 dark:text-slate-100 hover:bg-emerald-100/60 dark:hover:bg-emerald-900/50 border border-emerald-100 dark:border-emerald-900/40'
                      }`}
                    >
                      <div className="flex items-center space-x-2 truncate">
                        <span className="text-sm leading-none">{lang.flag}</span>
                        <div className="truncate">
                          <p className={`text-xs ${isSelected ? 'text-white' : 'text-slate-900 dark:text-white'} font-semibold truncate`}>
                            {lang.native}
                          </p>
                          <p className={`text-[10px] ${isSelected ? 'text-emerald-100' : 'text-slate-500 dark:text-slate-400'} truncate`}>
